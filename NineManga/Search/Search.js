...
for (manga of document.querySelectorAll('.bookinfo')) {
    let image = manga.querySelector('img').src;
    image = new ModuleRequest(image, 'get', emptyKeyValue, null);
    let title = manga.querySelector('.bookname').textContent;
    let desc = manga.querySelector('dd > p').textContent;
    let chapName = manga.querySelector('.chaptername').textContent;
    let views = manga.querySelector('span').textContent;
    let link = manga.querySelector('a').href + '?waring=1';
    link = new ModuleRequest(link, 'get', emptyKeyValue, null);
    searchArray.push( new Data(image, title, desc, chapName, views,'','', false, link));
}
