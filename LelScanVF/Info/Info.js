...
var episodes = [];
var type = 'Manga';
var status = document.querySelector('.label.label-success,.label.label-danger').textContent.trim();
var genres = [];
genres = Array.from(document.querySelectorAll('.dl-horizontal a')).map(g => g.textContent);
var desc = '';
try {
    desc = document.querySelector('div > div > p').textContent.trim();
} catch {}
var title = document.querySelector('body > div.wrapper > div img').alt;
var image = 'https:' + document.querySelector('body > div.wrapper > div img').src;
image = new ModuleRequest(image, 'get', emptyKeyValue, null);
var chapters = document.querySelectorAll('.chapters li');
if (chapters.length == 0) {
    let chapter = new Chapter('PAS D\'ÉPISODE DISPO', new ModuleRequest('', 'get', emptyKeyValue, null),
        false);
    episodes.push(chapter);
} else {
    for (var x = 0; x < chapters.length; x++) {
        for (var x = 0; x < chapters.length; x++) {
            if (chapters[x].className.includes('volume btn btn-default btn-xs')) {} else {
