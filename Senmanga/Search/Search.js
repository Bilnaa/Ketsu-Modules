...
var lastAdded = document.querySelector('.listupd').querySelectorAll('.upd');
let lastAddedArray = [];
for(var x = 0; x < lastAdded.length; x++) {
       var last = lastAdded[x];
       var image = last.querySelector('img').src;
       image = new ModuleRequest(image, 'get', emptyKeyValue, null);
       let title = last.querySelector('img').alt;
       var link = last.querySelector('a').href;
       link = new ModuleRequest(link, 'get', emptyKeyValue, null);
       var lastchap = 'Last chapter : ' + last.querySelector('.chapter').textContent.trim().replace('Ch.','');
       let data = new Data(image, title, lastchap, 'Manga', '', '', '', false, link);
