    ...
    const DefaultLayouts = {
        ...
        var ind = document.querySelectorAll('.ml-item');
        for (i of ind){
            var qual = i.querySelector('.mli-quality').textContent;
            var link = i.querySelector('a').href;
            link = new ModuleRequest(link, 'get', emptyKeyValue, null);
            var image = i.querySelector('img').src;
            image = new ModuleRequest(image, 'get', emptyKeyValue, null);
            var title = i.querySelector('.mli-info h2').textContent;
            try{
                var epi = i.querySelector('.mli-eps').textContent;
            }catch(e){
                var epi = '';
            }
            var finalData = new Data(image, title, epi, qual, '', '', '', false, link);;
            donnees.push(finalData);
        }
        var don = donnees.filter(function(me) {
            if(me.description === ''){
                unique.push(me)
            }
        });
        ...
        output.push(new Output(CellDesings.wide11, Orientation.vertical, DefaultLayouts.none, Paging.none, new Section('', false), testLayout, unique));
