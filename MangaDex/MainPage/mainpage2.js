...
for (slide of slides) {
    var title = slide.attributes.title;
    if (title == null) {} else {
        ...
        try {
            var desc = 'Chapter ' + slide.attributes.chapter;
        } catch (e) {
            var desc = '';
        }
        var statut = slide.attributes.status;
        var relationships1 = slide.relationships;
        ...
        dataArray.push(new Data(image, title + ' ' + desc, desc, '', '', '', '', false, link));
    }
}
var testLayout = new Layout(new Insets(0, 0, 10, 10), 1, 2, 3, 5, 300, new Size(0, 0), null, new Size(0, 0), 10, 0);
output.push(new Output(CellDesings.small2, Orientation.horizontal, DefaultLayouts.none, Paging.leading, new Section('Last Updates', true), testLayout, dataArray));
let MainPageObject = new MainPage(new ModuleRequest('https://api.mangadex.org/manga?limit=20&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&order[createdAt]=desc&includes[]=cover_art', 'get', emptyKeyValue, null), new Extra([new Commands('', emptyKeyValue)], emptyKeyValue), new JavascriptConfig(true, false, ''), output);
