...
var status = 'Unknown';
var genres = [];
var desc = document.querySelector('.wp-content').innerText.trim();
...
if (!parsedJson.request.url.includes('film')) {
    ...
    for (epi of donnes) {
        ...
        tout = epi.querySelector('.numerando').textContent;
        var sai = `Saison ${tout}`;
...
new Output(image, title, parsedJson.request, desc, genres, status, type, lang, 'Eps: ' + episodes.length, episodes));
